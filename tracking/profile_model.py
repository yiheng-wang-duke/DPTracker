...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='dptrack_b', choices=['dptrack_t', 'dptrack_b'],
...
def evaluate_vit(model, template, search):
    ...
    if args.script == 'dptrackt':
        macs1, params1 = profile(model, inputs=(template, search, [], []),
                                 custom_ops=None, verbose=False)
    elif args.script == 'dptrackb':
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            if args.script == 'dptrack_t':
                _ = model(template, search, [], [])
            elif args.script == 'dptrack_b':
                _ = model(template, search)
            else:
                return
        start = time.time()
...
if __name__ == "__main__":
    ...
    if args.script == "dptrack_t":
        ...
        model_constructor = model_module.build_dptrackt
        ...
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_dptrackb
        model = model_constructor(cfg, training=False)
        total_params = sum(p.numel() for p in model.parameters())
        print('total_params:', total_params)
        # get the template and search
