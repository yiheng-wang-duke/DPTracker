...
class Attention_with_QKV_Addition(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
    ...
    def forward(self, x, add_q=None, add_k=None, add_v=None, mask=None, return_attention=False):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        if add_q is not None:
            add_q = add_q.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
            q = torch.cat([q, add_q], dim=-1)
        if add_k is not None:
            add_k = add_k.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
            k = torch.cat([k, add_k], dim=-1)
        if add_v is not None:
            add_v = add_v.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
            v = torch.cat([v, add_v], dim=-1)
        assert q.shape[-1] == k.shape[-1]
        _C = q.shape[-1]
        attn = (q @ k.transpose(-2, -1)) * (_C ** -0.5)
        if mask is not None:
            attn = attn.masked_fill(mask.unsqueeze(1).unsqueeze(2), float('-inf'),)
...
class Attention_with_Bias(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
    ...
    def forward(self, x, bias=None, mask=None, return_attention=False):
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale
        # print("Attn Mean:", attn.mean().item())
        # print("Attn Std:", attn.std().item())

        if bias is not None:
            attn += bias
            # print("Bias Mean:", bias.mean().item())
            # print("Bias Std:", bias.std().item())
