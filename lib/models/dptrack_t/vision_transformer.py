...
from lib.models.dptrack_t.utils import combine_tokens, recover_tokens
...
class Block(nn.Module):
    def __init__(
        ...
        if self.use_adaptor:
            self.viewblock = AdaptorBlock(dim, dropout=drop_path)
            self.illublock = AdaptorBlock(dim, dropout=drop_path)

    def forward(self, x: torch.Tensor, illu: torch.Tensor, view: torch.Tensor) -> torch.Tensor:
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.illumination_prompter = IlluminationPrompter(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
    ...
    def _intermediate_layers(
        ...
        illu_x = self.illumination_prompter(x)
        view_x = self.view_prompter(x)
        x = self._pos_embed(x)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
        for i, blk in enumerate(self.blocks):
            x, illu_x, view_x = blk(x, illu_x, view_x)
    ...
    def forward_features(self, z, x, is_distill):
        illu_z = self.illumination_prompter(z)
        illu_x = self.illumination_prompter(x)
        view_z = self.view_prompter(z)
        view_x = self.view_prompter(x)
        ...
        illu = combine_tokens(illu_z, illu_x)
        view = combine_tokens(view_z, view_x)
        ...
        for i, blk in enumerate(self.blocks):
            if i > 1 and not is_distill:
                ...
                active_mask = prob_active > 0.5
                if len(idx) > 0:
                    if self.training:
                        x_all_processed, illu_all_processed, view_all_processed = blk(x, illu, view)
                        x = torch.where(active_mask.unsqueeze(-1), x_all_processed, x)
                        illu = torch.where(active_mask.unsqueeze(-1), illu_all_processed, illu)
                        view = torch.where(active_mask.unsqueeze(-1), view_all_processed, view)
                    else:
                        x, illu, view = blk(x, illu, view)
