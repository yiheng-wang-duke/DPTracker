...
def run_training(script_name, config_name, cudnn_benchmark=True, local_rank=-1, save_dir=None, base_seed=None,
    ...
    if base_seed is not None:
        if local_rank != -1:
            print('base_seed:', base_seed, "init_seed:", base_seed + local_rank)
...
def main():
    ...
    if int(torch_version.split('.')[0]) >= 2:
        ...
        print('using torch with version >= 2.0, set local rank:', args.local_rank)
    num_devices = torch.cuda.device_count()
    print(f'cuda count devices:{num_devices} gpus')

    if args.local_rank != -1 and args.local_rank < num_devices:
        print('start distributed training')
        dist.init_process_group(backend='nccl')
        torch.cuda.set_device(args.local_rank)
        print(f'local rank {args.local_rank} distributed training inited')
    else:
        print(f"[Warning] Invalid local_rank={args.local_rank}, fallback to cuda:0")
